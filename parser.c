    ...
    struct parser_exec_decl_braces_arg *arg)
{
	...
	if ((arg->flags & PARSER_EXEC_DECL_BRACES_FLAG_INDENT) ||
	...
	for (;;) {
		...
		if ((arg->flags & PARSER_EXEC_DECL_BRACES_FLAG_ENUM) ||
...
next:
		if (((arg->flags & PARSER_EXEC_DECL_BRACES_FLAG_ENUM) == 0) &&
    ...
    struct parser_exec_func_proto_arg *arg)
{
	...
	if ((s == All || s == TopLevel) ||
	    ((arg->flags & PARSER_EXEC_FUNC_PROTO_FLAG_IMPL) &&
	...
	parser_exec_attributes(pr, dc, &arg->out,
...
parser_exec_stmt_return(struct parser *pr, struct doc *dc)
{
	...
	if (!lexer_peek_if(lx, TOKEN_SEMI, NULL)) {
		...
		error = parser_exec_expr(pr, concat, NULL, NULL,
		    w, EXPR_EXEC_FLAG_INDENT | EXPR_EXEC_FLAG_NOPARENS);
		if (error & HALT)
			return parser_fail(pr);
	}
	if (lexer_expect(lx, TOKEN_SEMI, &tk))
...
parser_exec_stmt_expr(struct parser *pr, struct doc *dc)
{
	...
	error = parser_exec_expr(pr, dc, &expr, NULL,
	    style(pr->pr_st, ContinuationIndentWidth), EXPR_EXEC_FLAG_INDENT);
	if (error & HALT)
		return parser_fail(pr);
	if (lexer_expect(lx, TOKEN_SEMI, &semi))
		doc_token(semi, expr);
