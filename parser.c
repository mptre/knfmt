	...
	for (;;) {
		...
		if (lexer_if(lx, TOKEN_IDENT, &tk)) {
			...
			if (lexer_peek_if(lx, TOKEN_LBRACE, NULL)) {
				...
				struct token *stop;
				unsigned int eflags = EXPR_EXEC_FLAG_INDENT;
    ...
    struct parser_exec_decl_braces_arg *pb)
{
	...
	braces = doc_alloc(DOC_CONCAT, pb->dc);
	...
	for (;;) {
		...
		if ((pb->flags & PARSER_EXEC_DECL_BRACES_FLAG_ENUM) ||
			...
			error = parser_exec_decl_cppx(pr, concat, pb->rl);
			if (error & HALT)
				return parser_fail(pr);
		} else {
			struct token *stop;
			...
			error = parser_exec_expr(pr, concat, &expr, stop, 0);
			if (error & HALT)
				return parser_fail(pr);
		}
		if (lexer_if(lx, TOKEN_COMMA, &comma)) {
			...
			if (align) {
				...
				ruler_insert(pb->rl, comma, concat,
				    pb->col, w, 0);
				w = 0;
				goto next;
			}
...
next:
		if (((pb->flags & PARSER_EXEC_DECL_BRACES_FLAG_ENUM) == 0) &&
...
parser_exec_decl_cppx(struct parser *pr, struct doc *dc, struct ruler *rl)
{
	...
	for (;;) {
		...
		error = parser_exec_expr(pr, arg, &expr, stop,
		    EXPR_EXEC_FLAG_INDENT | EXPR_EXEC_FLAG_ARG);
		if (error & HALT)
			return parser_fail(pr);
    ...
    const struct token *type)
{
	...
	pf.dc = doc_alloc(DOC_CONCAT, doc_alloc(DOC_GROUP, dc));
    ...
    const struct token *type)
{
	struct parser_exec_func_proto_arg pf = {
		...
		.rl	= rl,
		.type	= type,
		.flags	= PARSER_EXEC_FUNC_PROTO_FLAG_IMPL,
	};
	struct lexer *lx = pr->pr_lx;
	int error;
...
parser_exec_func_proto(struct parser *pr, struct parser_exec_func_proto_arg *pf)
{
	...
	if (pf->type->tk_flags & TOKEN_FLAG_TYPE_FUNC) {
...
parser_exec_stmt1(struct parser *pr, struct doc *dc)
{
	struct parser_exec_stmt_block_arg ps = {
		.head	= dc,
		.tail	= dc,
		.flags	= PARSER_EXEC_STMT_BLOCK_FLAG_TRIM,
		.rbrace	= NULL,
	};
...
parser_exec_stmt_block(struct parser *pr, struct parser_exec_stmt_block_arg *ps)
{
	 ...
	 * declaration.
	 */
	if ((ps->flags & PARSER_EXEC_STMT_BLOCK_FLAG_TRIM) ||
...
parser_exec_stmt_return(struct parser *pr, struct doc *dc)
{
	...
	if (!lexer_peek_if(lx, TOKEN_SEMI, NULL)) {
		...
		error = parser_exec_expr(pr, concat, NULL, NULL,
		    EXPR_EXEC_FLAG_INDENT | EXPR_EXEC_FLAG_NOPARENS);
		if (error & HALT)
			return parser_fail(pr);
	}
	if (lexer_expect(lx, TOKEN_SEMI, &tk))
...
parser_exec_stmt_expr(struct parser *pr, struct doc *dc)
{
	...
	error = parser_exec_expr(pr, dc, &expr, NULL, EXPR_EXEC_FLAG_INDENT);
	if (error & HALT)
		return parser_fail(pr);
	if (lexer_expect(lx, TOKEN_SEMI, &semi))
		doc_token(semi, expr);
    ...
    const struct token *type, unsigned int flags)
{
	...
	if (lexer_peek_if(lx, TOKEN_LBRACE, NULL)) {
		...
		if (type->tk_type == TOKEN_SWITCH)
			ps.flags |= PARSER_EXEC_STMT_BLOCK_FLAG_SWITCH;
		doc_literal(" ", expr);
...
parser_exec_stmt_case(struct parser *pr, struct doc *dc)
{
	...
	if (kw->tk_type == TOKEN_CASE) {
		...
		error = parser_exec_expr(pr, lhs, NULL, NULL,
		    EXPR_EXEC_FLAG_INDENT);
		if (error & HALT)
			return parser_fail(pr);
	}
...
parser_exec_stmt_asm(struct parser *pr, struct doc *dc)
{
	...
	if (!lexer_peek_until(lx, TOKEN_COLON, &colon))
		return parser_fail(pr);
	error = parser_exec_expr(pr, opt, NULL, colon, 0);
	if (error & HALT)
		return parser_fail(pr);
	...
	if (lexer_if(lx, TOKEN_COLON, &tk)) {
		...
		if (!lexer_peek_if(lx, TOKEN_RPAREN, NULL))
			doc_alloc(DOC_LINE, concat);
		error = parser_exec_expr(pr, concat, NULL, rparen, 0);
		if (error & HALT)
			return parser_fail(pr);
	}
    ...
    struct ruler *rl)
{
	...
	if (pr->pr_op->op_flags & OPTIONS_FLAG_SIMPLE) {
		struct token *tk = beg;
		int ntokens = 0;
...
parser_simple_stmt_enter(struct parser *pr)
{
	...
	if ((pr->pr_op->op_flags & OPTIONS_FLAG_SIMPLE) == 0)
		return 0;
