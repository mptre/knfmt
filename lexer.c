    ...
    const struct token *tk)
{
	...
	t = token_alloc(lx->lx_arg.priv_size, tk);
	t->tk_off = st->st_off;
	t->tk_lno = st->st_lno;
	t->tk_cno = st->st_cno;
...
lexer_copy_after(struct lexer *lx, struct token *after, const struct token *src)
{
	...
	tk = token_alloc(lx->lx_arg.priv_size, src);
    ...
    const char *str)
{
	...
	tk = token_alloc(lx->lx_arg.priv_size, &(struct token){
	    .tk_type	= type,
...
lexer_branch_fold(struct lexer *lx, struct token *src)
{
	...
	prefix = token_alloc(lx->lx_arg.priv_size, &(struct token){
	    .tk_type	= TOKEN_CPP,
